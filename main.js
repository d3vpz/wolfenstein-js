...
// load wall textures
for(var file=0;file<TEXTURE_COUNT;file++){
...
function game_loop(){
    ...
    context.drawImage(
        ...
        0,1152,
        64,64,
        canvas.width/2-HALF_WIDTH,canvas.height/2-HALF_HEIGHT,
        WIDTH,HEIGHT
    );
    ...
    let offset_x=Math.floor(canvas.width/2)-HALF_WIDTH;
    let offset_y=Math.floor(canvas.height/2)-HALF_HEIGHT;
    ...
    for(var ray=0;ray<WIDTH;ray++){
        ...
        var ray_end_x,ray_end_y,ray_dir_x,vertical_depth,texture_end_y,texture_y;
        ...
        for(var offset=0;offset<MAP_RANGE;offset+=MAP_SCALE){
            ...
            if(map[target_square]!=0){texture_y=map[target_square];break;}
            ray_end_x+=ray_dir_x*MAP_SCALE;
        }texture_end_y=ray_end_y;

        // horizontal line intersection
        var ray_end_y,ray_end_x,ray_dir_y,horiz_depth,texture_end_x,texture_x;
        ...
        for(var offset=0;offset<MAP_RANGE;offset+=MAP_SCALE){
            ...
            var target_square=map_target_y*MAP_SIZE+map_target_x;
            if(target_square<0||target_square>map.length-1)break;
            if(map[target_square]!=0){texture_x=map[target_square];break};
        ...
        var texture_image=vertical_depth<horiz_depth?texture_y:texture_x;
        var texture_offset=vertical_depth<horiz_depth?texture_end_y:texture_end_x;
        texture_offset=Math.floor(texture_offset-Math.floor(texture_offset/MAP_SCALE)*MAP_SCALE);
        depth*=Math.cos(player.angle-current_angle);

        var wall_height=Math.min(Math.floor(MAP_SCALE*300/(depth+0.0001)),50000);
        // context.beginPath();
        // context.strokeStyle=vertical_depth<horiz_depth?'darkgray':'gray';
        // context.lineWidth=2;
        ...
        context.drawImage(
            ...
            texture_offset,0,
            1,64,
            ray+offset_x,(HALF_HEIGHT-Math.floor(wall_height/2))+offset_y,
            1,wall_height,
        );
    ...
    context.fillRect(0,0,canvas.width,offset_y);
    context.fillRect(0,offset_y+HEIGHT,canvas.width,canvas.width-offset_y+200)
